...
Bogeys are inferred per-leg pass events from v_bogey_events. A positive bogey means our runner passed another team during that leg; a negative/passed-by event means another team crossed us during that leg. Do not split bogeys by division. When start offsets are missing, call them same-start inferred and explain that wave starts can affect whether the computed crossing was a physical on-course pass.
