-- Remove invalid all-zero parsed split rows and make bogey events depend only on
-- team results with a complete, positive seven-leg split set.
DELETE FROM public.historical_leg_splits AS split
USING (
  SELECT team_result_id
  FROM public.historical_leg_splits
  GROUP BY team_result_id
  HAVING count(*) FILTER (WHERE split_time_seconds IS NOT NULL) >= 7
     AND COALESCE(sum(split_time_seconds), 0) = 0
) AS invalid_team
WHERE split.team_result_id = invalid_team.team_result_id;

CREATE OR REPLACE VIEW public.v_bogey_events AS
WITH valid_team_results AS (
  SELECT team_result_id
  FROM public.historical_leg_splits
  GROUP BY team_result_id
  HAVING count(*) FILTER (WHERE split_time_seconds IS NOT NULL) >= 7
  ...
  JOIN valid_team_results AS valid
    ON valid.team_result_id = team.id
